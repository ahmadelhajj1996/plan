...
const Employees = () => {
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          const employeesArray = Object.entries(employeesObject).map(
            ([id, employee]) => ({
              id,
              ...employee,
            })
      ...
      <div>
        <ul>
          {periodsArray.map(({ startDate, endDate, days }, index) => (
            <li key={index}>
              <strong>
                {startDate.slice(0, 5)} - {endDate.slice(0, 5)} =
              </strong>
              ({days})
            </li>
          ))}
  ...
  const initialValues = employeeToEdit
    ? {
        ...
        preferredShifts: Array.isArray(employeeToEdit.preferredShifts)
          ? employeeToEdit.preferredShifts
          : [],
        periodWantedDays: Array.isArray(employeeToEdit.periodWantedDays)
          ? employeeToEdit.periodWantedDays
          : [],
      }
    : {
        name: "",
        ...
        days: yup
          ...
          .test(
            ...
            (value) => {
              const numericValue = Number(value);
              return (
                !isNaN(numericValue) && numericValue >= 0 && numericValue <= 31
              );
            }
          ),
      })
    ...
    <div className="-ms-32 me-32 mt-16">
      <div className="flex justify-between items-center pb-4">
        <button
          ...
          className="w-[120px] bg-color"
        >
          {t("generate_plan")}
        </button>
        <div
          className="text-4xl text-end text-rose-500 italic cursor-pointer"
      ...
      >
        <div className="flex justify-between items-center focus:rounded-none pb-4 border-b sticky top-0 bg-white z-10">
          <h2>{employeeToEdit ? t("edit_subject") : t("add_subject")}</h2>
          <span
            className="cursor-pointer text-2xl text-rose-600"
            onClick={() => {
              setIsOpen(false);
              setEmployeeToEdit(null);
            }}
                  ...
                  <FieldArray name="periodWantedDays">
                    {({ push, remove }) => (
                      <div>
                        ...
                        <div className="-space-y-6 -mt-6">
                          {formik.values.periodWantedDays?.map(
                            (period, index) => (
                              <div
                                key={index}
                                className="flex gap-x-4 items-center justify-center"
      ...
      <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
        ...
        {data?.map((employee, index) => (
          <div
            key={index}
            className="grid grid-cols-7 w-full gap-x-4 border-b bordered"
          >
            ...
            <div className="whitespace-nowrap px-6 py-4">
              {employee.want_night ? "True" : "False"}
            </div>
      ...
      >
        ...
        <div className="mt-8 flex justify-between gap-x-8">
          <button
            className="bg-color hover:bg-color-hovered"
