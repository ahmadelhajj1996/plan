...
export function generateWorkPlan(employees, minEmployeesPerShift, monthIndex = new Date().getMonth()) {
  ...
  const periodConstraints = {}; // Track period wanted days constraints
  const periodAssignedDays = {}; // Track assignments within constrained periods
  ...
  function assignShift(employee, day, shiftType, isProvision = false) {
    ...
    // Check period constraints first
    ...
    // Track period assignments if applicable
  ...
  return { workPlan, shiftCounts, weekendShifts, nightShiftDays, periodAssignedDays };
}
